...
def scheme_read(src):
    """Read the next expression from SRC, a Buffer of tokens.
       返回读取到的下一个表达式:5 Types:Number,Symbol,Boolean,Combination,nil
    ...
    """
    if src.current() is None: # 检查src的第一个元素
        raise EOFError
    ...
    if val == 'nil':
        ...
        return nil
        # END PROBLEM 1
    elif val == '(':           # Combination
        # BEGIN PROBLEM 1
        "*** YOUR CODE HERE ***"
        return read_tail(src)
        # END PROBLEM 1
    elif val == "'":           # Symbol -> quotation
        # BEGIN PROBLEM 6
        "*** YOUR CODE HERE ***"
        return Pair('quote',Pair(scheme_read(src),nil))
        # END PROBLEM 6
    elif val not in DELIMITERS: # Symbol
        return val
    else:
