...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    """
    # Evaluate atoms
    if scheme_symbolp(expr):    # 变量名
        return env.lookup(expr)
    elif self_evaluating(expr): # 自求值
        return expr

    # All non-atomic expressions are lists (combinations)
    if not scheme_listp(expr): # 判断是否为合法的列表
        raise SchemeError('malformed list: {0}'.format(repl_str(expr)))
    first, rest = expr.first, expr.rest
    if scheme_symbolp(first) and first in SPECIAL_FORMS: # special-forms
        return SPECIAL_FORMS[first](rest, env)
    else: # call expression first = "fn"
        # BEGIN PROBLEM 4
        "*** YOUR CODE HERE ***"
        operator = scheme_eval(first, env) # operator实质上即为func
        validate_procedure(operator)       # to validate that the operator evaluates to a procedure
        # 计算operands
        args = rest.map(lambda x: scheme_eval(x, env)) # 在求当前表达式之前,需要对所有的子表达式都求值
        return scheme_apply(operator, args, env)
        # END PROBLEM 4

def self_evaluating(expr): # 判断是否可以自求值:boolean\number\null\string;symbol=>environment
...
def scheme_apply(procedure, args, env):
    ...
    environment ENV."""
    validate_procedure(procedure) # 检查procesdure是否是合法的 Procedure
    if isinstance(procedure, BuiltinProcedure):
        return procedure.apply(args, env)
    else: # 自定义函数:lambda and fns
        new_env = procedure.make_call_frame(args, env) # build a new frame !
        return eval_all(procedure.body, new_env)       # 在new_env中执行procedure的主体部分

def eval_all(expressions, env):
    """Evaluate each expression in the Scheme list EXPRESSIONS in
    environment ENV and return the value of the last.
    => the excution of the body of a function
    ...
    value = None
    while expressions != nil: # 注:通过迭代找到最后一个表达式的值
        value = scheme_eval(expressions.first, env)
        expressions = expressions.rest
    return value # 返回最后一个表达式的值
    # END PROBLEM 7
...
class Frame(object): # 代表一个Frame,多个Frame(Parent\Child)组成一个Environment
    ...
    def lookup(self, symbol): # Frame的查找
        """Return the value bound to SYMBOL. Errors if SYMBOL is not found."""
        # BEGIN PROBLEM 2
        "*** YOUR CODE HERE ***"
        if symbol in self.bindings:
            return self.bindings[symbol]
        elif self.parent != None:    # 递归:实现链式的传递
    ...
    def make_child_frame(self, formals, vals):
        ...
        """
        if len(formals) != len(vals): # 检查参数是否合法
            raise SchemeError('Incorrect number of arguments to function call')
        # BEGIN PROBLEM 10
        "*** YOUR CODE HERE ***"
        child_frame = Frame(self) # self的 child frame
        while formals != nil:
            child_frame.define(formals.first,vals.first) # 逐个绑定参数
...
class BuiltinProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        # BEGIN PROBLEM 3
        "*** YOUR CODE HERE ***"
        while args is not nil: # 将Pair类添加到列表中
            python_args.append(args.first)
            args = args.rest
        if self.use_env:  # 如果该函数需要当前环境,则在最后添加
...
class LambdaProcedure(Procedure):
    ...
    def make_call_frame(self, args, env): # 注:以self.env为 parent建立一个新的frame
        """Make a frame that binds my formal parameters to ARGS, a Scheme list
        of values, for a lexically-scoped call evaluated in environment ENV."""
        # BEGIN PROBLEM 11
        "*** YOUR CODE HERE ***"
        return self.env.make_child_frame(self.formals, args) # note: self.env为父环境
        # END PROBLEM 11
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target): # 说明是定义变量
        validate_form(expressions, 2, 2) # Checks that expressions is a list of length exactly 2
        # BEGIN PROBLEM 5
        "*** YOUR CODE HERE ***"
        symbol, value = expressions.first, scheme_eval(expressions.rest.first,env)
        env.define(symbol,value) # 去当前帧中定义symbol变量
        return symbol
        # END PROBLEM 5
    elif isinstance(target, Pair) and scheme_symbolp(target.first):
        # BEGIN PROBLEM 9
        "*** YOUR CODE HERE ***"
        formals = target.rest     # 得到fn的参数列表
        validate_formals(formals) # 检查参数
        body = expressions.rest   # 得到fn的body部分
        #(define (f x y) <body>) <=> (define f (lambda (x y) <body>))
        env.define(target.first, LambdaProcedure(formals, body, env)) # 在环境中将Lambda表达式与该fn名称绑定
        return target.first
        # END PROBLEM 9
    else:
...
def do_begin_form(expressions, env):
    ...
    正好是一个函数的执行过程: 执行每一步,并返回最后一个表达式的值
    >>> env = create_global_frame()
...
def do_lambda_form(expressions, env):
    ...
    # BEGIN PROBLEM 8
    "*** YOUR CODE HERE ***"
    body = expressions.rest  # 注: body is a scheme list of body expressions
...
def do_and_form(expressions, env):
    ...
    "*** YOUR CODE HERE ***"
    value = True               # 默认为 True
    while expressions != nil:  # Pair结构 => 遍历 Pair: rest->first->rest...
        value = scheme_eval(expressions.first, env)
        if is_false_primitive(value):  # 短路运算
            return False
        expressions = expressions.rest
    return value               # value为最后的一个表达式的值
    # END PROBLEM 12
...
def do_cond_form(expressions, env):
    ...
    while expressions is not nil:
        ...
        if is_true_primitive(test):
            # BEGIN PROBLEM 13
            "*** YOUR CODE HERE ***"
            if clause.rest == nil: # 如果没有对应的表达式,则返回 True
                return test        # return the predicate value
            return eval_all(clause.rest, env) # 计算所有表达式, 并返回最后一个的值
            # END PROBLEM 13
        expressions = expressions.rest
...
def do_let_form(expressions, env):
    ...
    """
    validate_form(expressions, 2)
    let_env = make_let_frame(expressions.first, env) # 返回创建的local frame
    return eval_all(expressions.rest, let_env) # 计算 body部分: let()语句只有body部分可以得到局部定义的变量
...
def make_let_frame(bindings, env):
    ...
    "*** YOUR CODE HERE ***"
    while bindings != nil:
        binding = bindings.first               # 依次检查每一个 binding
        validate_form(binding, 2,2)  # 检查 binding的格式
        names, values = Pair(binding.first, names), Pair(scheme_eval(binding.rest.first, env), values) # 利用自身进行迭代
        bindings = bindings.rest
    validate_formals(names)                    # 检查symbol是否重复
    # END PROBLEM 14
...
class MuProcedure(Procedure):
    ...
    def make_call_frame(self, args, env):
        """该动态函数的 parent frame is the environment in which the func is evaluated"""
...
def do_mu_form(expressions, env):
    ...
    # BEGIN PROBLEM 18
    "*** YOUR CODE HERE ***"
    body = expressions.rest           # 先不计算func的body部分, 符合解释器原理
    return MuProcedure(formals, body) # MuProcedure的 attribute中没有env
    # END PROBLEM 18

SPECIAL_FORMS['mu'] = do_mu_form # 在speical_forms中加入了do_mu_form
...
def read_eval_print_loop(next_line, env, interactive=False, quiet=False,
    ...
    while True:
        try:
            src = next_line()
            while src.more_on_line:                   # 保证全部scheme_read()完
                expression = scheme_read(src)         # 返回 Pair
                result = scheme_eval(expression, env) # 根据当前的环境计算result
