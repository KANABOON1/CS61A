...
class Name(Expr):
    ...
    def eval(self, env):
        ...
        """
        "*** YOUR CODE HERE ***"
        if self.var_name in env:  # Name的实例化对象求值:直接从环境中寻找
...
class LambdaExpr(Expr):
    ...
    def eval(self, env): # Lambda Expr的value是一个Lambda Function.
...
class CallExpr(Expr):
    ...
    def eval(self, env):
        ...
        """
        "*** YOUR CODE HERE ***"
        fn = self.operator.eval(env)  # operator对应的函数: Primitive()
        args = [e.eval(env) for e in self.operands] # operands对应的值: Number(n)
...
class LambdaFunction(Value):
    ...
    def apply(self, arguments):
        ...
        "*** YOUR CODE HERE ***"
        new_env=self.parent.copy() # 创建全局环境的一个副本,表示不同的帧frame
        new_env.update(dict(zip(self.parameters,arguments))) # zip:将两个可迭代对象的对应位置打包成为一个元组
        return self.body.eval(new_env)  # self.body在read时已经被初始化成为相应的Expr,故直接调用其method:eval()即可
