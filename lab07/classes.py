...
class Player:
    def __init__(self, deck, name):
        ...
        self.name = name
        "*** YOUR CODE HERE ***"
        self.hand=[deck.draw() for _ in range(5)]  # 随机从deck上抽取5张卡片
    ...
    def play(self, card_index):
        ...
        """
        "*** YOUR CODE HERE ***"
        return self.hand.pop(card_index)   # 列表基本操作pop,返回值即为删除的元素的值
...
class TutorCard(Card):
    ...
    def effect(self, other_card, player, opponent):
        ...
        opponent.hand[:3]=[]   # 利用切片操作删除列表元素
        opponent.hand.extend([opponent.deck.draw() for _ in range(3)])
...
class ProfessorCard(Card):
    ...
    def effect(self, other_card, player, opponent):
        ...
        for element in opponent.deck.cards.copy():   # 在循环中更改列表很可能会出错,因此需要copy一份不变的列表
            if element.attack==other_card.attack or element.defense==other_card.defense:
                opponent.deck.cards.remove(element)
