...
def closer_city(lat, lon, city_a, city_b):
    ...
    'Bucharest'
    """
    kana=make_city('kana',lat,lon)
    if distance(kana,city_a)-distance(kana,city_b)<=0:
...
def berry_finder(t):
    """
    有关于 tree的递归,tree的递归的基本形式:先看label,后逐个看branches=>for b in branches(tree):
    ...
    1
      2
        3
          ...
          2
    """
    if is_leaf(t):#注:即使是leave,也要tree(leave)不要违反抽象障碍!
...
def coords(fn, seq, lower, upper):
    ...
    """
    "*** YOUR CODE HERE ***"
    return [[ele,fn(ele)] for ele in seq if fn(ele)>=lower and fn(ele)<=upper]
...
def riffle(deck):
    ...
    """
    "*** YOUR CODE HERE ***"#注:index//2可以使得两个有相同性质的数据产生同样的值.
...
def add_trees(t1, t2):
    ...
    if is_leaf(t1):  # base case1
        ...
        mul_tree += branches(t1)[len_tree:]
        mul_tree += branches(t2)[len_tree:]
